...
| `BP-Net/` | <https://github.com/kakaxi314/BP-Net> | 基础模型仓 + 工作目录（数据 / checkpoints / outputs） |
| `.venv/` | (uv 创建) | 共享 Python 3.9 虚拟环境，host 与 BP-Net 都用它 |

三者都被 `.gitignore` 排除，互不干扰。
...
cd workspace
bash setup_rtx4060.sh           # 安装 uv（如缺）+ 在 ./.venv 装环境 + 下数据/权重
